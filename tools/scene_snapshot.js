"use strict";
// D:\Dev\pepakura-next\tools\scene_snapshot.js
Object.defineProperty(exports, "__esModule", { value: true });

const fs = require("fs");
const sceneRuntime = require("D:\\Dev\\pepakura-next\\src\\sceneRuntime.js");

const loadObjModel = sceneRuntime.loadObjModel;
const setCamera = sceneRuntime.setCamera;
const setModelMatrix = sceneRuntime.setModelMatrix;
const setLights = sceneRuntime.setLights;
const applySeams = sceneRuntime.applySeams;
const applyMeshData = sceneRuntime.applyMeshData;
const getCurrentModelPath = sceneRuntime.getCurrentModelPath;
const getCurrentCamera = sceneRuntime.getCurrentCamera;
const getCurrentModelMatrix = sceneRuntime.getCurrentModelMatrix;
const getCurrentLights = sceneRuntime.getCurrentLights;
const getCurrentSeams = sceneRuntime.getCurrentSeams;
const getCurrentMeshPositions = sceneRuntime.getCurrentMeshPositions;
const getCurrentMeshUVs = sceneRuntime.getCurrentMeshUVs;
const getCurrentMaterials = sceneRuntime.getCurrentMaterials;
  ...
  for (const rawLine of lines) {
    ...
    if (line.startsWith("matrix ")) {
      ...
      snapshot.matrix = rest.length
        ? rest.split(/\s+/).map((vv) => parseFloat(vv))
        : [];
      continue;
    }
    ...
    if (line.startsWith("seams ")) {
      ...
      snapshot.seams = rest.length
        ? rest.split(/\s+/).map((vv) => parseInt(vv, 10))
        : [];
      continue;
    }
...
function loadSceneSnapshot() {
  ...
  if (!fs.existsSync(snapshotPath)) {
    console.error("Спросшот не найден: " + snapshotPath);
    return;
  }
