...
const fs = require("fs");
const sceneRuntime_1 = require("D:\\Dev\\pepakura-next\\src\\modules\\renderer-3d\\sceneRuntime");
...
function saveSceneSnapshot(modelPathArg) {
    const runtimeModelPath = modelPathArg || (0, sceneRuntime_1.getCurrentModelPath)();
    const currentCamera = (0, sceneRuntime_1.getCurrentCamera)();
    const currentMatrix = (0, sceneRuntime_1.getCurrentModelMatrix)();
    const currentLights = (0, sceneRuntime_1.getCurrentLights)();
    const currentSeams = (0, sceneRuntime_1.getCurrentSeams)();
    const currentPositions = (0, sceneRuntime_1.getCurrentMeshPositions)();
    const currentUVs = (0, sceneRuntime_1.getCurrentMeshUVs)();
    const currentMaterials = (0, sceneRuntime_1.getCurrentMaterials)();
    const snapshot = {
        version: "1.0",
        timestamp: Date.now(),
        ...
        materials: currentMaterials.length > 0 ? currentMaterials : [{ color: "ffffff" }],
    };
...
function loadSceneSnapshot() {
    ...
    (0, sceneRuntime_1.loadObjModel)(snapshot.modelPath);
    (0, sceneRuntime_1.setCamera)(snapshot.camera.position, snapshot.camera.target);
    (0, sceneRuntime_1.setModelMatrix)(snapshot.matrix);
    (0, sceneRuntime_1.setLights)(snapshot.lights.ambient, snapshot.lights.dir);
    (0, sceneRuntime_1.applySeams)(snapshot.seams);
    (0, sceneRuntime_1.applyMeshData)(snapshot.meshesPositions, snapshot.meshesUVs);
...
function main() {
    ...
    if (args.length === 0) {
        ...
        console.log("  node D:\\Dev\\pepakura-next\\tools\\scene_snapshot.js save D:\\Dev\\pepakura-next\\models\\head.obj");
        console.log("  node D:\\Dev\\pepakura-next\\tools\\scene_snapshot.js load");
        return;
    }
    ...
    if (command === "save") {
        ...
        if (!modelPathArg || !modelPathArg.startsWith("D:\\")) {
            console.error("Ошибка: ожидается абсолютный путь к модели, начинающийся с D:\\");
            return;
        }
