...
import MeshOptimizer from '../components/MeshOptimizer.vue'
import PdfExporter from '../components/export/PdfExporter.vue'
import { usePdfExport, type UnfoldedMesh } from '../composables/usePdfExport'
...
const { invoke, isDesktop, isWeb, init } = usePlatform()

const isTauriAvailable = isDesktop

// === UI State ===
const activeView = ref<'3d' | '2d' | 'ai' | 'settings'>('3d')
...
const import3D = async () => {
  ...
  try {
    if (!isTauriAvailable) {
      ...
      input.type = 'file'
      input.accept = '.obj,.stl,.ply,.fbx,.gltf,.glb,.pdo'
      input.onchange = async (event) => {
        ...
        // В веб-режиме просто сохраняем имя файла как modelPath (заглушка)
        store.modelPath = file.name
        store.projectName = file.name.replace(/\.[^.]+$/, '')
        activeView.value = '3d'
        showMessage('3D модель загружена (веб-режим, функциональность ограничена)', 'success')
        isLoading.value = false
      }
      input.click()
      // Ждем, пока пользователь выберет файл (обработчик выше)
      return
    }
...
const unfoldModel = async () => {
  ...
  try {
    if (!isTauriAvailable) {
      // В веб-режиме используем платформенный мост
      // Если команда не поддерживается, покажем сообщение
      showMessage('Развёртка в веб-режиме временно недоступна', 'warning')
      isLoading.value = false
      return
    }
...
const exportResult = async (format: string) => {
  ...
  try {
    if (!isTauriAvailable) {
      // Веб-режим: сохраняем файл через браузер
      const blob = new Blob([store.svgContent], { type: 'image/svg+xml' })
      ...
      showMessage(`Экспортировано как export.${format}`, 'success')
      return
    }
...
// PDF экспорт с диалогом настроек
const showPdfExporter = ref(false)
const { exportPdf, defaultSettings } = usePdfExport()

// Получаем развёртку из store для экспорта
const getUnfoldedMesh = (): UnfoldedMesh | null => {
  // Пока возвращаем заглушку - в реальности нужно парсить из store.unfoldedResult
  // или получить от бэкенда через invoke
  if (!store.svgContent) return null
...
const exportPDF = async () => {
  ...
  if (!unfolded) {
    showMessage('Сначала создайте развёртку', 'warning')
    return
  }
  showPdfExporter.value = true
}

const handlePdfExported = (path: string) => {
  showMessage(`PDF экспортирован: ${path.split(/[\\/]/).pop()}`, 'success')
}
...
const saveProject = async () => {
  try {
    if (!isTauriAvailable) {
      // Веб-режим: сохраняем как JSON файл
      const projectData = {
      ...
      const a = document.createElement('a')
      a.href = url
      a.download = `${store.projectName || 'project'}.pepa.json`
      document.body.appendChild(a)
      a.click()
      document.body.removeChild(a)
      URL.revokeObjectURL(url)
      store.markSaved()
      showMessage('Проект сохранён (веб-режим)', 'success')
      return
    }
    const { save } = await import('@tauri-apps/plugin-dialog')
...
onMounted(async () => {
  ...
  if (!isTauriAvailable) {
    console.warn('Tauri API недоступен. Приложение запущено в браузере.');
  }
  // Инициализируем платформенный мост (уже инициализирован в App.vue, но на всякий случай)
  try {
    await init();
  } catch (error) {
...
<template>
  <div class="editor-layout" v-bind="$attrs">
      ...
      <div class="viewer-section">
        ...
        <div class="viewer-container">
          ...
          <template v-else-if="activeView === 'ai'">
            <AiGenerator style="width: 100%; height: 100%;" />
          </template>
          ...
          <div class="viewer-stats" v-if="modelPath || svgContent">
            ...
            <span v-else-if="activeView === 'ai'">AI Генерация 3D из изображения</span>
          </div>
        </div>
      </div>
  ...
  <!-- PDF Export Dialog -->
  <PdfExporter
    v-if="showPdfExporter"
    :unfolded="getUnfoldedMesh()"
    @close="showPdfExporter = false"
    @exported="handlePdfExported"
  />
</template>
