<template>
  ...
  import { useModelViewer } from '@pepakura/shared/composables/useModelViewer'
  import { useModelFileLoader } from '@pepakura/shared/composables/useModelFileLoader'
  import { useModelStats } from '@pepakura/shared/composables/useModelStats'
  import type { MeshData } from '@pepakura/shared/types/model'
  
  const store = useProjectStore()
  const modelPath = computed(() => store.modelPath)
  const modelInfo = computed(() => store.modelInfo)
  ...
  async function readFileAsArrayBuffer(path: string): Promise<ArrayBuffer> {
    if (isTauriAvailable.value) {
      // Используем Tauri API для чтения файла
      const { readFile } = await import('@tauri-apps/plugin-fs')
      const data = await readFile(path)
      return data.buffer
    } else {
      // В браузере используем fetch, но если путь file://, преобразуем в URL
      let url = path
      if (path.startsWith('file://')) {
        // В браузере file:// не работает, нужно использовать другой метод
        // Для простоты оставим fetch, но это вызовет ошибку
        console.warn('Загрузка file:// в браузере невозможна')
      }
  ...
  watch(modelPath, async (newPath) => {
    if (newPath && isSceneReady()) {
      try {
        ...
        const arrayBuffer = await readFileAsArrayBuffer(newPath)
        ...
        // Обновляем информацию об ошибке в сторе
        store.modelInfo = {
          vertices: 0,
          faces: 0,
          edges: 0,
